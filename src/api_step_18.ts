...
>() {
	static readonly Live = Layer.effect(
		...
		// This `pipe` could also be applied at the `LiveLayer` level on the service, but it is good practice to directly provide the required dependencies when defining a `Layer`
...
const liveProgram = program.pipe(Effect.provide(LiveLayer));
